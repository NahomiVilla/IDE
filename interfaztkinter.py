...
def seleccion_graficos(grafi,cant):
    global selec_graph,selec_cant
    selec_graph =grafi
    selec_cant=cant  

def seleccion_puertos(ard,bd):
    global seleccion_arduino,seleccion_baudio
    seleccion_arduino=ard
    seleccion_baudio=bd

def click_conexion():
    global img2,conexion_exitosa,usb_boton
    if puertos_manejo.conexion!=True:
        click_conectar(seleccion_arduino,seleccion_baudio,selec_graph,selec_cant)
...
def configuracion():
    global fuente,siz,tema,temas,modo
    ...
    label_fuente = tk.Label(frame_fuentes, text="Font",font=('Arial',12),bg='black',fg='white')
    label_fuente.pack(side=tk.LEFT)
    label_tamaño = tk.Label(frame_tamaño, text="Size",bg='black',fg='white',font=('Arial',12))
    label_tamaño.pack(side=tk.LEFT)
    label_tema = tk.Label(frame_tema, text="Theme",bg='black',fg='white',font=('Arial',12))
    label_tema.pack(side=tk.LEFT)
    label_modo = tk.Label(frame_modo, text="Mode",bg='black',fg='white',font=('Arial',12))
...
for arduino in arduinos :
    baudios_menu=tk.Menu(menu_puertos,tearoff=0)
    for baudio in baudios:
        baudios_menu.add_command(label=baudio, command=lambda arduino=arduino, baud=baudio: seleccion_puertos(arduino,baud)) 
    menu_puertos.add_cascade(label=arduino,menu=baudios_menu)
